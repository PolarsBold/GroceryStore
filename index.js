...
function logNames(items) {
  ...
  items.forEach((object) => {
    return newArray.push(object.name);
  });
...
function getUppercaseNames(items) {
  return items.map((object) => object.name.toUpperCase());
}
...
function getItemsByCategory(items, category) {
  return items.filter((object) => object.category === category);
}
...
function countItems(items) {
  return items.reduce(
    (runningTotal, object) => (runningTotal += object.quantity),
    0
  );
}
...
function getTotalPrice(items) {
  return items.reduce(
    (runningTotal, object) => (runningTotal += object.price * object.quantity),
    0
  );
}
